      ...
      <div className="max-w-3xl mx-auto text-center relative z-10">
        ...
        <FadeIn delay={0.2}>
          <p className="text-neutral-400 text-lg mb-3 leading-relaxed max-w-xl mx-auto">
            {t.description}
            <span className="text-red-400 font-medium">{t.descriptionHighlight}</span>
            {t.descriptionSuffix}
          </p>
          <div className="flex flex-wrap justify-center gap-x-6 gap-y-2 text-sm text-neutral-500 mb-8">
            {t.checks.map((check) => (
              <span key={check} className="flex items-center gap-1.5">{CHECK} {check}</span>
            ))}
          </div>
        </FadeIn>
        ...
        <FadeIn delay={0.4}>
          <div className="mt-8 flex flex-col items-center gap-3">
            <p className="text-neutral-600 text-xs uppercase tracking-widest">{t.preferText}</p>
            <motion.a
              href={whatsappUrl}
              target="_blank"
